document.getElementById("item-containers").addEventListener("click",function(e){
     if(e.target.className.includes("add-cart-btn")){
        const title= e.target.parentNode.children[1].children[1].innerHTML
          const price=Number(e.target.parentNode.children[1].children[2].children[0].innerHTML)
          const cartImg=e.target.parentNode.children[0].children[0].src
